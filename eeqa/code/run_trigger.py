...
from pytorch_pretrained_bert.modeling import (
    BertForTriggerClassification,
    BertLSTMForTriggerClassification,
)
...
candidate_queries = [
    [
        ...
        "event",
        "?",
    ],  # 0 what is the trigger in the event?
            ...
            with open(file) as f:
                for line in f:
                    ...
                    if len(sentence) > self.max_sent_length:
                        self.max_sent_length = len(sentence)
                    for event in events:
...
def read_ace_examples(nth_query, input_file, tokenizer, category_vocab, is_training):
    ...
    with open(input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            for event in events:
                # only use the first token of the trigger
...
def evaluate(
    args, eval_examples, category_vocab, model, device, eval_dataloader, pred_only=False
):
    ...
    for idx, (
        ...
        with torch.no_grad():
            logits = model(
                input_ids, token_type_ids=segmend_ids, attention_mask=input_mask
            )
        ...
        for i, in_sent in enumerate(in_sentence):
            ...
            for idj, j in enumerate(in_sent):
                if j:
                    decoded_tag_seg.append(
                        category_vocab.index_to_category[tag_seq[idj]]
                    )
            sentence_triggers = []
    ...
    # get results (event identification)
    gold_events = []
    for sent_triggers in gold_triggers:
        sent_events = []
        for trigger in sent_triggers:
            sent_events.append(trigger[1])
        gold_events.append(sent_events)
    ...
    for sentence_id in pred_triggers:
        ...
        for trigger in pred_triggers[sentence_id]:
            sent_events.append(trigger[1])
        pred_events.append(sent_events)

    gold_trigger_n, pred_trigger_n, true_positive_n = 0, 0, 0
    for sentence_id, _ in enumerate(pred_events):
        gold_sent_events = gold_events[sentence_id]
        pred_sent_events = pred_events[sentence_id]
        # for pred_trigger_n
        for trigger in pred_sent_events:
            pred_trigger_n += 1
        # for gold_trigger_n
        for trigger in gold_sent_events:
            gold_trigger_n += 1
        # for true_positive_n
        common = collections.Counter(gold_sent_events) & collections.Counter(
            pred_sent_events
        )
        true_positive_n += sum(common.values())

    prec_e, recall_e, f1_e = 0, 0, 0
    if pred_trigger_n != 0:
        prec_e = 100.0 * true_positive_n / pred_trigger_n
    else:
        prec_e = 0
    if gold_trigger_n != 0:
        recall_e = 100.0 * true_positive_n / gold_trigger_n
    else:
        recall_e = 0
    if prec_e or recall_e:
...
def main(args):
    ...
    if args.do_train or (not args.eval_test):
        eval_examples, eval_features = read_ace_examples(
        ...
        if args.add_lstm:
            eval_features = sorted(
                eval_features, key=lambda f: np.sum(f.input_mask), reverse=True
            )
        logger.info("***** Dev *****")
        logger.info("  Num orig examples = %d", len(eval_features))
        logger.info("  Batch size = %d", args.eval_batch_size)
        all_sentence_id = torch.tensor(
            [f.sentence_id for f in eval_features], dtype=torch.long
        )
        all_input_ids = torch.tensor(
            [f.input_ids for f in eval_features], dtype=torch.long
        )
        all_segmend_ids = torch.tensor(
            [f.segment_ids for f in eval_features], dtype=torch.long
        )
        all_in_sentence = torch.tensor(
            [f.in_sentence for f in eval_features], dtype=torch.long
        )
    ...
    if args.do_train:
        _, train_features = read_ace_examples(
            nth_query=args.nth_query,
        ...
        if args.add_lstm:
            train_features = sorted(
                train_features, key=lambda f: np.sum(f.input_mask), reverse=True
            )
        ...
        all_in_sentence = torch.tensor(
            [f.in_sentence for f in train_features], dtype=torch.long
        )
        all_input_mask = torch.tensor(
        ...
        train_data = TensorDataset(
            all_input_ids, all_segment_ids, all_input_mask, all_labels
        )
        ...
        for lr in lrs:
            if args.add_lstm:
                model = BertLSTMForTriggerClassification.from_pretrained(
                    args.model,
                    cache_dir=PYTORCH_PRETRAINED_BERT_CACHE,
                    num_labels=len(category_vocab.index_to_category),
                )
            else:
                model = BertForTriggerClassification.from_pretrained(
                    args.model,
                    cache_dir=PYTORCH_PRETRAINED_BERT_CACHE,
                    num_labels=len(category_vocab.index_to_category),
                )
            ...
            if not args.add_lstm:
                ...
                optimizer = optim.SGD(
                    model.parameters(), lr=args.lstm_lr, momentum=0.9, weight_decay=1e-6
                )
                    ...
                    if (step + 1) % eval_step == 0 or step == 0:
                        ...
                        if args.do_eval:
                            ...
                            if (best_result is None) or (
                                ...
                                logger.info(
                                    "!!! Best dev %s (lr=%s, epoch=%d): p_c: %.2f, r_c: %.2f, f1_c: %.2f, p_i: %.2f, r_i: %.2f, f1_i: %.2f, p_e: %.2f, r_e: %.2f, f1_e: %.2f"
                                    % (
                                        args.eval_metric,
                                        str(lr),
                                        epoch,
    ...
    if args.do_eval:
        if args.eval_test:
            ...
            all_labels = torch.tensor(
                [f.labels for f in eval_features], dtype=torch.long
            )
        ...
        # BertForTriggerClassification.from_pretrained(args.model, cache_dir=PYTORCH_PRETRAINED_BERT_CACHE, num_labels=len(category_vocab.index_to_category))
            ...
            if args.add_lstm:
                model = BertLSTMForTriggerClassification.from_pretrained(
                    model_dir, num_labels=len(category_vocab.index_to_category)
                )
            else:
                model = BertForTriggerClassification.from_pretrained(
                    model_dir, num_labels=len(category_vocab.index_to_category)
                ...
                if args.fp16:
                    model.half()
                model.to(device)
                result, preds = evaluate(
                    args, eval_examples, category_vocab, model, device, eval_dataloader
                )

                with open(os.path.join(model_dir, "test_results.txt"), "w") as writer:
                    for key in result:
                        writer.write("%s = %s\n" % (key, str(result[key])))
                with open(os.path.join(model_dir, "trigger_predictions.json"), "w") as writer:
                    for line in preds:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        parser.add_argument(
            "--model_dir",
            default="trigger_qa_output/epoch0-step0",
            type=str,
            required=True,
        ...
        parser.add_argument(
            "--add_lstm",
            action="store_true",
            help="Whether to add LSTM on top of BERT.",
        )
        parser.add_argument(
            "--lstm_lr",
            default=None,
            type=float,
            help="The initial learning rate for lstm Adam.",
        )
        parser.add_argument(
            "--nth_query", default=0, type=int, help="use n-th candidate query"
        )
