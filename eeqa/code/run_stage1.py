...
from pytorch_pretrained_bert.modeling import (
    ...
    BertForQuestionAnswering_withIfTriggerEmbedding,
)
...
class AceExample(object):
    ...
    def __repr__(self):
        ...
        for event in self.events:
            if event:
                # This essentially does: sentence[event.start] event.type event.offset
                # - event[0][0] is the start of the event since s_start is always zero
                # - event[0][-1] (por event[0][2]) is the event type
                # - event[0][1] is not used by PHEE because PHEE only uses the first
                #   token of the event as the trigger, so event[0][1] == event[0][0]
                #
                # For example, if the sentence is "Physicians are doctors" and
                # the event is "Physicians" with type "profession", then this
                # will be "Physicians profession 0"
...
def read_ace_examples(input_file, is_training):
    ...
    with open(input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            sentence, events, s_start, instance_id = (
                example["sentence"],
                example["event"],
                example["s_start"],
                example["id"],
...
def convert_examples_to_features(
    examples, tokenizer, query_templates, nth_query, is_training
):
        ...
        for event in example.events:
            ...
            for argument_type in query_templates:
                ...
                for i, token in enumerate(example.sentence):
                    ...
                    if sub_tokens:
                        # Ignore sub-tokens. Use only the first sub-token.
                ...
                if is_training:
                    ...
                    for argument in arguments:
                        ...
                        if gold_argument_type == argument_type:
                            ...
                            start_position = (
                                answer_start - sentence_start + sentence_offset
                            )
...
def read_query_templates(normal_file):
    ...
    with open(normal_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # 2 template arg_name + in trigger (replace [trigger] when forming
            # the instance)
            query_templates[arg_name].append(arg_name + " in [trigger]")
            # 3 template arg_query
            query_templates[arg_name].append(query)
            # 4 arg_query + event type
            query_templates[arg_name].append(query[:-1] + " in [event_type]?")
            # 5 arg_query + trigger (replace [trigger] when forming the instance)
            query_templates[arg_name].append(query[:-1] + " in [trigger]?")
...
def make_predictions(
    ...
    for example_id, _ in enumerate(all_examples):
        ...
        for feature in features:
            event_type_argument_type = ".".join(
                [feature.event_type, feature.argument_type]
            )
            event_type_offset_argument_type = ".".join(
                [
                    feature.event_type,
                    str(feature.token_to_orig_map[feature.fea_trigger_offset]),
                    feature.argument_type,
                ]
            )
            ...
            for result in results:
                if (
                    result.event_type_offset_argument_type
                    == event_type_offset_argument_type
                ):
                    ...
                    # Find the top-N predictions (by logit score) that don't overlap
                    # with previous predictions for this example.
                    ...
                    for pred in prelim_predictions:
                        ...
                        if pred.start_index == 0 and pred.end_index == 0:
                            break
                        orig_sent_start, orig_sent_end = (
                            feature.token_to_orig_map[pred.start_index],
                        ...
                        for k_type, k_offset in final_all_predictions[example_id]:
                            if k_type == event_type_argument_type:
                                if (
                                    orig_sent_start >= k_offset[0]
                                    and orig_sent_start <= k_offset[1]
                                ) or (
                                    orig_sent_end >= k_offset[0]
                                    and orig_sent_end <= k_offset[1]
                                ):
                                    overlap = True
                                    break
                        idx += 1
...
def _get_best_indexes(logits, n_best_size=1, larger_than_cls=False, cls_logit=None):
    ...
    best_indexes = []
    for i, (index, score) in enumerate(index_and_score):
        if i >= n_best_size:
            break
        if larger_than_cls:
            if score < cls_logit:
...
def evaluate(
    ...
    for idx, (
        ...
        with torch.no_grad():
            if not args.add_if_trigger_embedding:
                ...
                batch_start_logits, batch_end_logits = model(
                    input_ids, segment_ids, if_trigger_ids, input_mask
                )
        for i, example_index in enumerate(example_indices):
            ...
            event_type_offset_argument_type = ".".join(
                [
                    eval_feature.event_type,
                    str(
                        eval_feature.token_to_orig_map[eval_feature.fea_trigger_offset]
                    ),
    ...
    for example_id, example in enumerate(gold_examples):
        ...
        for event in example.events:
            ...
            for argument in event[1:]:
                argument_start, argument_end, argument_type = (
                    argument[0] - example.s_start,
                    argument[1] - example.s_start,
                    argument[2],
                )
                argument_text = " ".join(sentence[argument_start : argument_end + 1])
                if argument_type in argument_types:
                    gold_rst[instance_id][event_type + "." + argument_type].append(
                        argument_text
                    )

    instances = []
    ins_ids = pred_rst.keys() | gold_rst.keys()
    ...
    for instance_id in ins_ids:
        ...
        question_types = preds.keys() | golds.keys()
...
def main(args):
    ...
    logger.info(
        "device: {}, n_gpu: {}, 16-bits training: {}".format(device, n_gpu, args.fp16)
    )
    ...
    if args.do_train:
        ...
        lrs = (
            ...
            else [1e-6, 2e-6, 3e-6, 5e-6, 1e-5, 2e-5, 3e-5, 5e-5]
        )
        for lr in lrs:
            if not args.add_if_trigger_embedding:
                    ...
                    if (step + 1) % eval_step == 0 or step == 0:
                        ...
                        if args.do_eval:
                            result, preds, _ = evaluate(
                                ...
                                eval_examples,
                                eval_features,
                                argument_types=list(query_templates.keys()),
                            )
                            model.train()
                            ...
                            if (best_result is None) or (
                                result[args.eval_metric] > best_result[args.eval_metric]
                            ):
                                train_best_model_dir = os.path.join(
                                    args.output_dir,
                                    "epoch{epoch}-step{step}".format(
                                ...
                                logger.info(
                                    "!!! Best dev %s (lr=%s, epoch=%d): CLS_EM_F1: %.2f, CLS_Micro_F1: %.2f"
                                    % (
                                        args.eval_metric,
                                        str(lr),
                                        epoch,
                                        result["CLS_Arguments_EM_F1"],
                                        result["CLS_Arguments_MICRO_F1"],
                                    )
                                )
                        ...
                        if save_model:
                            ...
                            if best_result:
                                ...
                                with open(
                                    os.path.join(subdir, "eval_results.txt"), "w"
                                ) as writer:
    ...
    if args.do_eval:
        ...
        if args.eval_dev:
            dev_result, dev_preds, _ = evaluate(
                args,
                model,
                device,
                eval_dataloader,
        ...
        if args.eval_test:
            eval_examples = read_ace_examples(
                input_file=args.test_file, is_training=False
            )
            gold_examples = read_ace_examples(
                input_file=args.gold_file, is_training=False
            )
            eval_features = convert_examples_to_features(
                examples=eval_examples,
                tokenizer=tokenizer,
            ...
            with open(args.test_file, "r") as f:
                for lid, line in enumerate(f.readlines()):
                    ...
                    for event in dat["event"]:
                        if event:
                            trigger_ofs, ev_type = event[0]
                            events[ev_type]["Trigger"].append(
                                [trigger_ofs, trigger_ofs, ev_type]
                            )
                    pred_rst = preds[lid]
                    for p in pred_rst:
                        ev_type, ag_type = p[0].split(".")
                        events[ev_type][ag_type].append([p[1][0], p[1][1], ag_type])
                    dat["event"] = []
                    for ev_type in events:
                        ev = []
                        for ag_type in ["Trigger", "Subject", "Treatment", "Effect"]:
                            if ag_type == "Trigger":
                                ev.append(events[ev_type][ag_type][0])
                            else:
                                for ag in events[ev_type][ag_type]:
                                    ev.append(ag)
                        dat["event"].append(ev)
                    output_lines.append(json.dumps(dat))
...
if __name__ == "__main__":
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        parser.add_argument(
            "--model_dir",
            default="args_qa_output/epoch0-step0",
            type=str,
            required=True,
        ...
        parser.add_argument(
            "--nth_query", default=0, type=int, help="use n-th template query"
        )
        parser.add_argument("--normal_file", default=None, type=str)
        parser.add_argument("--des_file", default=None, type=str)
        parser.add_argument(
            "--larger_than_cls", action="store_true", help="when indexing s and e"
        )
        parser.add_argument(
